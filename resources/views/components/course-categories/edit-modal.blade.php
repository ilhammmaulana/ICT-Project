<button class="btn btn-sm"
    onclick="document.getElementById('{{ str_replace('-', '_', $courseCategory->id) . '_edit' }}').showModal()"><svg
...
<dialog id="{{ str_replace('-', '_', $courseCategory->id) . '_edit' }}" class="modal">
        ...
        <form action="{{ route('admin.course-categories.update', $courseCategory) }}" method="POST">
            ...
            <div class="mt-5 mb-2">
                ...
                <input type="text" placeholder="Category name" class="input input-bordered w-full max-full mt-2"
                    id="module-name-input" name="name"value="{{ $courseCategory->name }}" />
            </div>


            <div class="flex gap-3 items-center justify-end mt-5">
                <button class="btn" type="button"
                    onclick="document.getElementById('{{ str_replace('-', '_', $courseCategory->id) . '_edit' }}').close()">Close</button>
